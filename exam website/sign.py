...
class SignUp:
    ...
    def signup_func(self):
        if self.fname_txt.get()=="" or self.lname_txt.get()=="" or self.email_txt.get()=="" or self.questions.get()=="Select" or self.answer_txt.get()=="" or self.password_txt.get() == "":
            ...
            try:
                connection = pymysql.connect(host="localhost", user="root", password="S@ndhani@1274#", database="re")
                cur = connection.cursor()
                cur.execute("select * from sr where email=%s",self.email_txt.get())
                row=cur.fetchone()
                ...
                if row!=None:
                    ...
                    cur.execute("insert into sr (f_name,l_name,email,question,answer,password) values(%s,%s,%s,%s,%s,%s)",
                                    (
                                        self.fname_txt.get(),
                                        self.lname_txt.get(),
                                        self.email_txt.get(),
