...
class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In PBRVITS")
        # Set the window size
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                ...
                cur.execute("select * from sr where email=%s and password=%s",(self.email_entry.get(),self.password_entry.get()))
                row=cur.fetchone()
                if row == None:
                    ...
                    messagebox.showinfo("Success","Wellcome to the PBRVITS family",parent=self.window)
    ...
    def forgot_func(self):
        if self.email_entry.get()=="":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                cur = connection.cursor()
                cur.execute("select * from sr where email=%s", self.email_entry.get())
                row=cur.fetchone()
                if row == None:
    ...
    def change_pass(self):
        if self.email_entry.get() == "" or self.sec_ques.get() == "Select" or self.new_pass.get() == "":
            ...
            try:
                connection = pymysql.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                cur = connection.cursor()
                cur.execute("select * from sr where email=%s and question=%s and answer=%s", (self.email_entry.get(),self.sec_ques.get(),self.ans.get()))
                row=cur.fetchone()
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("update sr set password=%s where email=%s", (self.new_pass.get(),self.email_entry.get()))
                        connection.commit()
